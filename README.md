...
1. Start PostgreSQL locally (optional - if not running, the application will automatically fall back to an in-memory H2 database).
2. Create the database (only if using local PostgreSQL):
...
The main datasource settings are read from environment variables first, then default to in-memory H2 values. When starting via Docker Compose, environment variables override the defaults to connect to PostgreSQL.

```properties
spring.datasource.url=${SPRING_DATASOURCE_URL:jdbc:h2:mem:search_typeahead;DB_CLOSE_DELAY=-1}
spring.datasource.username=${SPRING_DATASOURCE_USERNAME:sa}
spring.datasource.password=${SPRING_DATASOURCE_PASSWORD:}
```

### Force Seeding Dataset

By default, the application seeds the dataset on startup only if the database is empty. To force database truncation and a clean reload of all 333,333 queries from `unigram_freq.csv` at startup, set the `FORCE_SEED` environment variable to `true` (this is enabled by default in `docker-compose.yml` for testing and evaluation):
